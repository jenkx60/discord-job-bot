...
export async function POST(request) {
    try {
        ...
        const claimerBeforeCancel = job.assigned_user_id || parseDiscordUserId(candidateInfo) || job.assigned_user
        ...
        const canceledAttempt = await supabaseAdmin.rpc("increment_metric", {
            ...metricPayload,
            metric_type: "canceled",
        })
        metricError = canceledAttempt.error ?? null
        if (metricError) {
            const cancelledAttempt = await supabaseAdmin.rpc("increment_metric", {
                ...metricPayload,
                metric_type: "cancelled",
            })
            metricError = cancelledAttempt.error ?? null
        }
        ...
        if (repost) {
            ...
            const repostResult = await repostJob(supabaseAdmin, jobId, {
            ...
            const { data: jobAfter } = await supabaseAdmin.from("jobs").select("*").eq("id", jobId).single()
        ...
        // No repost: notify everyone still on the shortlist
